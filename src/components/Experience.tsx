  ...
  return (
    <section id="experiencia" className="py-24 px-4 sm:px-6 lg:px-8 bg-[#0F172A]/50">
      <div className="max-w-6xl mx-auto" ref={ref}>
        ...
        >
          <h2 className="text-3xl sm:text-4xl font-bold text-[#F1F5F9] mb-4">{t.experience.title}</h2>
          <div className="w-16 h-1 bg-[#3B82F6] mx-auto rounded-full mb-4" />
          <p className="text-[#94A3B8] max-w-xl mx-auto">{t.experience.subtitle}</p>
        </motion.div>
        ...
        <div className="relative">
          ...
          <div className="space-y-8">
            {experiences.map((exp, index) => {
              const translated = t.experience.entries[index];
              return (
                <motion.div
                  key={exp.company}
                ...
                >
                  ...
                  <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-6 hover:border-[#3B82F6]/40 transition-colors duration-300">
                    ...
                    <ul className="space-y-2">
                      {(translated?.activities ?? exp.activities).map((activity, i) => (
